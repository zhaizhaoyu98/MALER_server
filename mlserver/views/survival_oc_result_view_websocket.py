...
from mlserver.views.classification_oc_result_view_webscoket import get_file_md5, df2bp, mklinechart, split_train_test, \
...
def data_analysis(WebSocket,client_msg,projectid):
    ...
    analysis_results = oc_sur_analysis(client_msg,projectid)
...
def oc_sur_analysis(client_msg,projectid):
    try:
        ...
        if not os.path.exists(os.path.join(STATIC_ROOT, 'cache', projectid, 'surv_pickle.pkl')):
            ...
            if 'vsubplot_sur' in surv_pickle.keys():
                ifval = True
            else:
                ifval = False
        #send email
