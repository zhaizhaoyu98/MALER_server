...
患者级源码、机器可读结果、生成脚本与投稿图件已发布为 `v1.0.0-review4`；
后续输入校验与隐私提示已在两仓库 `master` 提交 `77d16ab`。正文和回复信区分
历史结果标签和后续源码提交。阿里云应用源码于 2026-09-24 更新到 `35aa116`；
...
缩放可覆盖的明确说明。此处列举的 `35aa116` 及以前的源码已部署；后续 `77d16ab` 修复尚未上云，不把页面级冒烟检查写成端到端上传验证。
...
- 2026-09-25 当前最终 clean 手稿已在逐字节副本上用 Word/Zotero Refresh；51 个字段和 81 条书目保留，刷新副本与正式稿 SHA-256 同为 `11E1B4A9CABC54814745B78FEC55C21B66494D4108F418F6CAFEAF124802E3C6`。正式稿未被 Word 改写；后续再动引文须重新 Refresh，勿 Unlink Citations。
...
- 审稿人 1 意见 6：发现分析上传只有页面 200 MB 文案而无入口校验。已在公开源码 `77d16ab` 的入口实际拒绝超限文件，新增本地测试；回复信区分已公开源码与此前云端版本，不再引用仅检查配置的旧安全审计来证明路由校验。
- 编辑次要意见 6：原始 CSV 重复样本列名会被 pandas 改成 `.1` 后绕过检查。已在读表前、分析上传预览和预测上传/结果入口检查原始表头，新增三项相关测试。
- 编辑主要意见 9：分析页补显性敏感数据警告；源码已公开，公网分析页仍未显示新增警告，回复信不把它描述成线上已生效。
...
本轮输入校验与隐私提示已在两仓库 `master` 提交 `77d16ab` 公开，云端应用
源码仍为此前的 `35aa116`；Word/LibreOffice 排版核验已完成。
