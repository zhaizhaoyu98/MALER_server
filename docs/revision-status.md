...
`v1.0.0-review3` 是此前公开基线，**不包含**本轮修正；不能把该标签说成
现行稿件对应的公开版本。代码、验证结果、环境文件、许可证和图表均有本地证据。
...
| 表格与图片 | 本地已更新 | Table 1-3、Table S1、Figure 1-3、Supplementary Figures S1-S6；Figure 2 已重抓缩放/预测隐私面板并移除旧“24 小时物理删除”截图，不再引用不存在的 S7 |
| 投稿 Word | 内容与结构已核对，视觉复核待作者完成 | clean/marked 正文与回复信、补充报告；当前工具会话未能调用 Word/LibreOffice 逐页渲染 |
| 新版本公开与阿里云同步 | 未完成 | 新修正尚未提交/推送/部署；原 review3 标签不能代替新版本 |
...
防火墙审计、静态存储加密、备份恢复演练，以及 review3 的云端部署。

## 投稿前仍需完成或由作者确认

1. 发布本轮源码、结果、脚本和图件为新的 GitHub/Gitee 版本，再核对公开链接；不要沿用旧标签。
2. 在 Word/Zotero 中执行一次 Refresh，不要 Unlink Citations，并逐页查看 clean/marked 正文、回复信、补充材料及 Table S1。当前自动渲染因缺少 LibreOffice 且 Word COM 不可用而未完成。
3. 核对作者、单位、基金、通讯作者、利益冲突和伦理声明，以及期刊图片格式、DPI、文件大小、上传类别。
4. 云端同步后复测界面、任务运行及上传/删除行为；维护期限与负责人应可真实履行，不承诺永久可用。
