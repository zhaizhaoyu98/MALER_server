...
患者级源码、验证结果和图件已公开为 `v1.0.0-review4`；此后的输入校验和隐私提示
修复已在 GitHub/Gitee `master` 提交 `77d16ab`，但未上云。`v1.0.0-review3`
是此前基线，**不包含**患者级修正。投稿 DOCX 仍位于本地包，不随代码仓库公开。
...
| 新版本公开 | 源码已公开；云端待部署 | 患者级结果在 GitHub/Gitee 的 `v1.0.0-review4`；新增上传大小、重复表头和分析页隐私修复已作为 `77d16ab` 推送两仓库，阿里云仍为此前的 `35aa116` |
...
1. 当前 clean 手稿已在逐字节副本上重新用 Word/Zotero Refresh，50 个引文、1 个书目字段、81 条书目保留；刷新副本与正式稿 SHA-256 同为 `11E1B4A9CABC54814745B78FEC55C21B66494D4108F418F6CAFEAF124802E3C6`。Word 和 LibreOffice 均已渲染本轮稿件，勿 Unlink Citations。
2. 核对作者、单位、基金、通讯作者、利益冲突和伦理声明，以及期刊图片格式、DPI、文件大小、上传类别。
3. 本轮三类修复已作为 `77d16ab` 公开，仍须部署到阿里云，并用非敏感合成数据复核分析上传超限、重复样本表头和两页隐私提示。若期刊要求实际线上任务证据，再做端到端上传/删除演练；生产定时清理、备份策略和维护负责人仍须运营方确认，不承诺永久可用。
