...
| 投稿 Word | 已完成 LibreOffice 与原生 Word 双重排版核验 | clean/marked 正文与回复信、补充报告及表格均已核验；Word 中四表各 1 页。Zotero Refresh 在与正式 clean 手稿逐字节相同的副本上执行后无改动，81 条书目及引文字段核验通过；作者仍须核对投稿元数据 |
| 新版本公开 | 已完成 | 患者级修正已推送 GitHub/Gitee 并标记 `v1.0.0-review4`；阿里云应用源码已同步到 `35aa116` |
...
1. Word/Zotero Refresh 已在正式 clean 手稿的逐字节副本上执行：50 个引文字段、1 个书目字段、81 条书目均正常，刷新前后文件 SHA-256 相同；正式文件因此未被重新保存。若作者之后编辑引文或改期刊样式，再用 Word 刷新，切勿 Unlink Citations。
2. 核对作者、单位、基金、通讯作者、利益冲突和伦理声明，以及期刊图片格式、DPI、文件大小、上传类别。
3. 如期刊要求实际线上任务证据，使用非敏感合成数据做端到端上传/删除演练；生产定时清理、备份策略和维护负责人仍须运营方确认，不承诺永久可用。
