...
- **实测结果**：`manage.py check` 无问题，**36/36 测试通过**，5 条公开路由全部返回 200，生产口径安全审计 **10/10**。
...
| 在范围内 | 不在范围内 |
| --- | --- |
| 启动 Django 站点、跑通 36 项测试 | 阿里云部署栈（`gunicorn`、`gevent`、Nginx） |
...
| 自动化测试 | `manage.py test mlserver -v 1` | **Ran 36 tests — OK** |
